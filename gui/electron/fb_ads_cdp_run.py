"""
CDP runner for Facebook Ads campaign creation.

Replaces fb_ads_mcp_run.py — uses the CDP-based server instead of MCP.
Same normalization logic, same input/output contract.
"""
import contextlib
...
def load_server_module(server_path: str):
    ...
    spec = importlib.util.spec_from_file_location("fb_ads_cdp_server_module", module_path)
    if spec is None or spec.loader is None:
        raise RuntimeError(f"No pude cargar el servidor CDP desde {module_path}")
...
def normalize_bundle_spec(payload: Dict[str, Any], progress) -> Tuple[Dict[str, Any], List[Dict[str, str]]]:
    ...
    _emit_rule(
        progress, rules, 1, total_steps, "runner.context",
        (
    ...
    if not ad_account_id:
        raise RuntimeError("Regla del runner: ad_account_id es obligatorio para crear la campana.")
    account_name, account_name_source, _ = _resolve_string(
        [
            ("account_name", source.get("account_name")),
            ("runner_context.execution.accountHint", execution_context.get("accountHint")),
        ],
        ad_account_id, "runner.default.account_name",
    )
    ...
    # Preserve cdp_port if present
    if source.get("cdp_port"):
        normalized["cdp_port"] = source["cdp_port"]

    # Preserve access_token if present
    if source.get("access_token"):
        normalized["access_token"] = source["access_token"]

    _emit_rule(progress, rules, 2, total_steps, "ad_account_id",
        f"Usando cuenta publicitaria {ad_account_id} desde {ad_account_source}; account_name='{account_name}' desde {account_name_source}.")
    _emit_rule(progress, rules, 3, total_steps, "page_id",
        f"Usando page_id {page_id} desde {page_id_source}." if page_id
        else f"Sin page_id utilizable; fuente revisada={page_id_source}.")
    ...
    ui_objective_label, ui_objective_source, ui_objective_fallback = _resolve_string(
        ...
        "Clientes potenciales", "runner.default.ui_objective_label",
    )
    ...
    _emit_rule(progress, rules, 4, total_steps, "campaign",
        f"name='{campaign_name}' desde {campaign_name_source}, objective='{normalized['campaign']['objective']}', status='{normalized['campaign']['status']}'.")
    ...
    ui_budget_mode, ui_budget_mode_source, _ = _resolve_string(
        [
        ...
        "Presupuesto diario", "runner.default.ui_budget_mode_label",
    )
    ui_conversion_location, ui_conversion_source, _ = _resolve_string(
        [
        ...
        "Formularios instantaneos", "runner.default.ui_conversion_location_label",
    )
    ui_performance_goal, ui_performance_goal_source, _ = _resolve_string(
        [
        ...
        "Maximizar el numero de clientes potenciales", "runner.default.ui_performance_goal_label",
    )
    ...
    _emit_rule(progress, rules, 5, total_steps, "adset.base",
        f"name='{adset_name}' desde {adset_name_source}, budget='{normalized['adset']['daily_budget'] or normalized['adset']['lifetime_budget']}'.")
    _emit_rule(progress, rules, 6, total_steps, "adset.targeting",
        f"Targeting desde {targeting_source}: {_targeting_summary(targeting)}.")
    _emit_rule(progress, rules, 7, total_steps, "adset.ui_flow",
        f"budget_mode='{ui_budget_mode}', conversion='{ui_conversion_location}', performance='{ui_performance_goal}'.")
    ...
    _emit_rule(progress, rules, 8, total_steps, "adset.promoted_object",
        f"promoted_object={json.dumps(promoted_object, ensure_ascii=False)}." if promoted_object else "Sin promoted_object.")

    if is_whatsapp:
        normalized["lead_form"] = None
        _emit_rule(progress, rules, 9, total_steps, "lead_form", "Modo WhatsApp: no se requiere formulario.")
        _emit_rule(progress, rules, 10, total_steps, "lead_form.required_fields", "Modo WhatsApp: sin campos.")
    else:
        required_fields, required_fields_source, required_fields_fallback = _resolve_string_list(
            ...
            DEFAULT_REQUIRED_FIELDS, "runner.default.required_fields",
            normalizer=_normalize_required_fields,
        )
        lead_form_page_id, lead_form_page_id_source, _ = _resolve_string(
            [("lead_form.page_id", lead_form.get("page_id")), (f"resolved.{page_id_source}", page_id)],
            page_id, "runner.default.lead_form_page_id",
        )
        lead_form_name, lead_form_name_source, _ = _resolve_string(
        ...
        _emit_rule(progress, rules, 9, total_steps, "lead_form",
            f"page_id='{lead_form_page_id}', form_id='{normalized['lead_form']['form_id'] or 'auto'}', name='{lead_form_name}'.")
        _emit_rule(progress, rules, 10, total_steps, "lead_form.required_fields",
            f"required_fields={required_fields} desde {required_fields_source}.")

    headline, headline_source, _ = _resolve_string(
        [
        ...
        "Lead Gen Draft", "runner.default.creative_headline",
    )
    message, message_source, _ = _resolve_string(
        [
    ...
    description, description_source, _ = _resolve_string(
        [
    ...
    link, link_source, _ = _resolve_string(
        [("creative.link", creative.get("link")), ("runner_context.preview.url", preview_context.get("url"))],
    ...
    creative_name, creative_name_source, _ = _resolve_string(
    ...
    object_story_spec, object_story_spec_source = _build_object_story_spec(
        _dict(creative.get("object_story_spec")), page_id, link, message, headline, description, cta_type,
    ...
    _emit_rule(progress, rules, 11, total_steps, "creative.headline", f"headline='{_truncate(headline)}' desde {headline_source}.")
    _emit_rule(progress, rules, 12, total_steps, "creative.message", f"message='{_truncate(message)}' desde {message_source}.")
    _emit_rule(progress, rules, 13, total_steps, "creative.detail",
        f"description='{_truncate(description)}', link='{_truncate(link)}', cta='{cta_type}', name='{creative_name}'.")
    _emit_rule(progress, rules, 14, total_steps, "creative.asset",
        f"image_path='{image_path}'." if image_path else "Sin image_path.")

    ad_name, ad_name_source, _ = _resolve_string(
        [("ad.name", ad.get("name"))],
        _build_named_fallback("Ad", segment_label, preview_start, preview_end) or "Ad Borrador",
        "runner.default.ad_name",
    )
    normalized["ad"] = {"name": ad_name, "status": _string(ad.get("status")) or "PAUSED"}
    _emit_rule(progress, rules, 15, total_steps, "ad", f"name='{ad_name}', status='{normalized['ad']['status']}'.")
    _emit_rule(progress, rules, 16, total_steps, "ui_flow.summary",
        f"modal='{normalized['campaign'].get('ui_objective_label', 'Clientes potenciales')}', budget='{ui_budget_mode}', conversion='{ui_conversion_location}'.")
    _emit_rule(progress, rules, 17, total_steps, "runner.summary",
        "Payload normalizado; ejecutando create_lead_campaign_bundle via CDP.")
...
def main() -> int:
    if len(sys.argv) < 2:
        print(json.dumps({"ok": False, "error": "Uso: fb_ads_cdp_run.py <fb_ads_cdp_server.py>"}))
        return 1
    ...
    try:
        progress("[RUNNER] Iniciando normalizacion del payload para el bundle de Meta Ads via CDP...")
