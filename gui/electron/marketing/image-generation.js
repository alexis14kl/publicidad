...
const { buildFullPrompt, lookupCompanyData } = require('../company/lookup')
const { prepareLatestMarketingImageAsset } = require('./image-asset')

const PROMPT_FILE = path.join(PROJECT_ROOT, 'utils', 'prontm.txt')
const DETECT_PORT_PY = path.join(PROJECT_ROOT, 'cdp', 'detect_port.py')
const FORCE_CDP_PY = path.join(PROJECT_ROOT, 'cdp', 'force_cdp.py')
const OPEN_PROFILE_JS = path.join(PROJECT_ROOT, 'perfil', 'abrir_perfil_dicloak.js')
const PROMPT_AUTOMATION_PY = path.join(PROJECT_ROOT, 'prompt', 'page_pronmt.py')
const DOWNLOAD_IMAGE_PY = path.join(PROJECT_ROOT, 'prompt', 'download_generated_image.py')
const OVERLAY_LOGO_PY = path.join(PROJECT_ROOT, 'utils', 'overlay_logo.py')
  ...
  try {
    ...
    if (fs.existsSync(OVERLAY_LOGO_PY)) {
      const overlayResult = await runPythonScript(
        pythonBin,
        OVERLAY_LOGO_PY,
        [path.join(PROJECT_ROOT, 'img_publicitarias')],
        env,
        onLog,
        '[IMAGE-AUTO][LOGO]'
      )
