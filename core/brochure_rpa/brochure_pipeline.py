...
from core.cfg.platform import BROCHURE_RPA_DIR, BROCHURES_DIR
...
def _resolve_logo() -> str:
    ...
    if logo and Path(logo).exists():
        return logo
    p = PROJECT_ROOT / "assets" / "logos" / "logoapporange.png"
    if p.exists():
        return str(p)
    for d in [PROJECT_ROOT / "assets" / "logos" / "companies", PROJECT_ROOT / "assets" / "logos"]:
        if d.exists():
...
def run_brochure_pipeline(cdp_port: int = 0, logo_path: str = "") -> int:
    ...
    from core.brochure_rpa.brochure_setup import run_brochure_setup
    ...
    from core.brochure_rpa.html_to_pdf import run_html_to_pdf
    rc = run_html_to_pdf(
